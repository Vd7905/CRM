...
export default function CustomerAnalytics() {
  const [customers] = useState(sampleCustomers); // replace with backend data prop/fetch
  ...
  const timelineData = useMemo(() => {
    // produce a friendly array for area chart - grouped by month
    const monthMap = {};
    customers.forEach((c) => {
    ...
    // sort by month key (lexicographic works for YYYY-MM)
    arr.sort((a, b) => (a.month > b.month ? 1 : -1));
    // include some filler to make chart look nice if empty
    if (arr.length === 0) {
      return [
  ...
  return (
    <main className="flex-1 p-4 sm:p-6 bg-[var(--background)] text-[var(--text)] transition-colors duration-300">
              ...
              <div style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={timelineData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="total" stroke="var(--green-primary)" fill="url(#colorTotal)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <aside className="space-y-6">
          ...
          <Card className="rounded-2xl bg-[var(--card)] border border-[var(--muted)]">
            ...
            <CardContent className="p-4 sm:p-6 space-y-3 text-[var(--text)]">
              {/* Flatten recommendations from customers (dedupe) */}
              {Array.from(
      ...
      {/* Selected customer quick drawer (lightweight) */}
